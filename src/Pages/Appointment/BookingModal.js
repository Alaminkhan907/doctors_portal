...
const handleBookings = (event) => { 
    ...
    return (
        <div><input type="checkbox" id="booking-modal" class="modal-toggle" />
            <div class="modal modal-bottom sm:modal-middle">
                <div class="modal-box">
                    <label for="booking-modal" class="btn btn-sm btn-circle absolute right-2 top-2">✕</label>
                    <h3 class="font-bold text-lg text-center text-bold">{name}</h3>
                    <form onSubmit={handleBookings} class="grid grid-cols-1 gap-3 justify-items-center mt-2">
                        ...
                        <input type="submit" value="submit" class="btn bg-gradient-to-r from-secondary to-primary w-full max-w-xs" />
                    </form>
                </div>
            </div>
        </div>
